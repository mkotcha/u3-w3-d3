...
const printArray = function (arr: (string | number | Cars)[]) {
...
const sortArray = function (arr: string[]) {
...
const sortArrayReverse = function (arr: string[]) {
...
const firstToLast = function (arr: string[]) {
  let item = arr.shift();
  if (item) {
    arr.push(item);
  }
...
const addKey = function (keyName: string, value: string, obj: Cars[]) {
...
const addToCars = function (brand: string, model: string, color: string, trims: string[], licensPlate: string) {
  const item = {
    brand: brand,
    model: model,
    color: color,
...
const printUntilNum = function (num: number) {
  let count = 0;
...
const charToNum = function (array: string[]) {
  const arrayNum = [];
